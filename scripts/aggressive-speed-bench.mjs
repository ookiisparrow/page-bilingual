/**
 * Real DeepSeek A/B: 1.4.61 baseline vs 1.4.62 aggressive speed.
 * Uses Chrome for Testing (supports --load-extension). Run:
 *   DEEPSEEK_API_KEY=... node scripts/aggressive-speed-bench.mjs
 */
...
const OUT = process.env.PBT_BENCH_OUT || "/cursor/stores/bc-0e734b8a-3c7b-404e-a396-bcc89b495698/media/aggressive-speed";
...
function build(variant) {
  ...
  execSync(`VARIANT=${variant} DEST=${dest} bash ${ROOT}/scripts/aggressive-speed-build.sh`, { stdio: "inherit" });
  return dest;
}

function launchChrome(extPath, port) {
  const profile = `/tmp/pbt-cft-${port}`;
...
async function benchVariant(variant, port) {
  ...
  try {
    ...
    return { variant, blocked: false, results, chromePid: chrome.pid };
  } finally {
...
const baseline = await benchVariant("baseline", 9230);
const current = await benchVariant("current", 9231);
const report = {
